...
const ForumPage = () => {
  ...
  const handleAdminAnswer = async (postId) => {
    ...
    try {
      const { data } = await axios.patch(
        `${API_BASE_URL}/api/forum/posts/${postId}/answer`,
        { answer },
        config
      );
  ...
  const handleAddComment = async (postId) => {
    ...
    try {
      ...
      setForumData(prev =>
        prev.map(p => p._id === postId ? { ...p, replyCount: (p.replyCount || 0) + 1 } : p)
      );
    ...
    <div className="forum-container">
            ...
            <motion.div className="modal-card" initial={{ y: 50 }} animate={{ y: 0 }}>
              ...
              <form onSubmit={handlePost}>
                <div className="form-row">
                  <select
                    value={newQuestion.tribe}
                    onChange={e => setNewQuestion({ ...newQuestion, tribe: e.target.value })}
                  >
                    {['All', 'Euro', 'JDM', 'Muscle', 'Classic'].map(t => (
                      <option key={t} value={t}>{t} Tribe</option>
                    ))}
                  </select>
                  ...
                  >
                    {['Discussion', 'Post Question', 'Build Log', 'Tech Help', 'FAQ'].map(c => (
      ...
      <aside className="forum-sidebar">
        <button
          ...
          onClick={() => user ? setIsModalOpen(true) : alert('Log in to post')}
        >
                    ...
                    {replies[post._id]?.map(reply => (
                      <div key={reply._id} className="reply-item">
                        <div className="reply-meta">
                          <b>{reply.author?.username}</b> • {getTimeAgo(reply.createdAt)}
                        </div>
                        <p>{reply.text}</p>
                      </div>
                    ))}
